...
// Shared optional fields for all registry types
const registrySharedFields = {
  host: z.string().optional(), // Optional, has defaults (gitlab.com, github.com)
  token: z.string().optional(), // Can also be set via env vars
  prefix: z.string().optional(), // Artifact name prefix (e.g., "frontend" → "frontend-artifact-name")
};

// Registry entry for local config (.grekt/config.yaml)
// Uses discriminated union: project is required for gitlab/github, optional for default
export const RegistryEntrySchema = z.discriminatedUnion("type", [
  z.object({
    type: z.literal("gitlab"),
    project: z.string(), // Required: GitLab project path (e.g., "myteam/artifacts")
    ...registrySharedFields,
  }),
  z.object({
    type: z.literal("github"),
    project: z.string(), // Required: GHCR namespace (e.g., "myorg")
    ...registrySharedFields,
  }),
  z.object({
